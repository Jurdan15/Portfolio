    ...
    public partial class FullyAnsweredBreakdown : ContentPage
    {
        ...
        private async Task LoadResponseStatistics(ChartView chartView, DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                string query = @"SELECT fully_answered FROM feedback 
                                 WHERE timestamp >= @startDate AND timestamp < @endDate AND fully_answered IS NOT NULL";
