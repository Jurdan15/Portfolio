...
// Handle save (add or replace existing)
if (isset($_POST['save'])) {
    ...
    // Delete existing rule with same rule_name
    $stmt = $conn->prepare("DELETE FROM rules WHERE rule_name = ? AND created_by = ?");
    $stmt->bind_param("ss", $rule_name, $username);
    $stmt->execute();
...
<table>
    ...
    <tr>
        ...
        ?>
            <button class="btn-small" onclick='editRow(<?= $ruleNameJs ?>, <?= $intentJs ?>, <?= $utterListJs ?>)'>Edit</button>
...
<script>
function editRow(ruleName, intent, utters) {
